    ...
    for (int i = 0; i < 8; i++)
    {
      for (int j = 0; j < 8; j++)
      {
        ...
        for (int k = 0; k < 4; k++)
        {
          ...
          if (getOreKindOfId(grid2No((Grid ) { x, y })) == iron
              || getOreKindOfId(grid2No((Grid ) { x, y })) == gold
              || getOreKindOfId(grid2No((Grid ) { x, y })) == diamond)
          {
            edge_w = 1;
          }
          else
...
{
  float weight = 1;
  bool enough_to_enhance = emerald >= 64
      || (emerald >= 32 && find_optimal_enhancement() != STRENGTH);
...
void update_mine()
{
  for (int i = 0; i < mineNum; i++)
  {
    if ((mineList[i].grid.x == nowGrid.x && mineList[i].grid.y == nowGrid.y)
        || (mineList[i].grid.x == opGrid.x && mineList[i].grid.y == opGrid.y))
    {
      mineList[i].store = 0;
    ...
    else
    {
      ...
      mineList[i].store = base
          * (time / 200 - mineList[i].last_visit_tick / 200);
    }
  }
}
