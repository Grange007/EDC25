/*注：注释未对齐确实不是编写代码的同学没有注意对齐，而是从CubeIDE粘贴过来之后已经对齐的注释出现错位了
 *反之，在notion代码块中对齐的注释粘贴回CubeIDE也会出现一样的情况，所以这里也没有重新对齐注释，
  ...
  while(idx < JY62_MESSAGE_LENGTH){                    //在判断JY62_MESSAGE_LENGTH(200)的长度内进行查找
    ...
    if(sum == jy62Receive[idx + 10]){                  //校验位正确，认为收到正确包
      ...
      idx++;                              //校验位错误，数据指针向后挪动，继续找到合理的数据包
		// Notice: A common mistake is to get a wrong package of data,
		// but still add 11 bits for next stage of searching.
    }
  }
